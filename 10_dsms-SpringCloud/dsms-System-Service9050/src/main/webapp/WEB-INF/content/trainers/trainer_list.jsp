											...
											<c:if test="${flag eq 'admin' }">
												<c:forEach items="${allTrainers }" var="trainer"
													...
													<tr>
														...
														<td><a
															href="trainer/showTrainerEdit/${trainer.id }"
															title="修改该教练信息" style="color: #2196f3">${trainer.trainerNo }</a>
														</td>
														<td>${trainer.name }</td>
														<td><c:if test="${trainer.gender eq 'M' }">男</c:if>
															<c:if test="${trainer.gender eq 'F' }">女</c:if></td>
														<td>${trainer.age }</td>
														<td>${trainer.phone }</td>
														<td>${trainer.idNumber }</td>
														<td>${trainer.licenseType }</td>
														<td></td>
														<td><fmt:formatDate value="${trainer.joinDate }"></fmt:formatDate></td>
														<td>${trainer.comments }</td>
														<td><a
															href="trainer/deleteTrainer/${trainer.id }"> <img
